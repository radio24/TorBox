    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for file in file_list:
                file_info = (f"{settings.MEDIA_ROOT}/{get_file_object_path(file)}",
                            file.name,
                            file.is_dir)
            ...
            # We have enought free space to go
            if total_size < free:
                ...
                response['Content-Disposition'] = f'attachment; filename=torbox-tfs_{random}.zip'
