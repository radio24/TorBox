...
# Additional to install: pip3 install pysocks
...
session.proxies = {}
session.proxies['http']='socks5h://192.168.42.1:9050'
session.proxies['https']='socks5h://192.168.42.1:9050'

bridges = False
# Try to fetch it over tor, if that doesn't work out, try to fetch directly
while bridges == False:
	try:
		res = session.get(tor_get_bridges_url).text
